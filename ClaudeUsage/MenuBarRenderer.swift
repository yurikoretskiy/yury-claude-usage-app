...
enum MenuBarRenderer {
    ...
    static func renderCompactMenuBarImage(percentage: Double, stale: Bool = false) -> NSImage {
        ...
        return NSImage(size: NSSize(width: width, height: height), flippable: false) { _ in
            ...
            // Pins inset 1px from canvas edges so 1pt stroke doesn't clip
            // (v2.22.4 fix — without fill, edge strokes lose half their width)
            let leftPinX: CGFloat = 1, rightPinX: CGFloat = 31
            ...
            // v2.22.4: hollow silhouette — fixed light white-gray outline so
            // it reads consistently regardless of system appearance mode.
            // Transparent interior. Digit stays orange.
