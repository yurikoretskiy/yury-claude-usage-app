...
enum MenuBarRenderer {
    ...
    static func renderCompactMenuBarImage(percentage: Double) -> NSImage {
        ...
        return NSImage(size: NSSize(width: width, height: height), flippable: false) { _ in
            ...
            // --- Pixel digits (v2.8): 3×5 bitmap at unit=1.5 for 1-2 digits,
            // unit=1 for 3-digit. Antialiasing on (within saved state) so 1.5
            // cells render bolder than a crisp 1-px stroke while glyph stays
            // small (~4.5×7.5 px, ~54% of body height).
            let pctNumber = "\(Int(round(percentage)))"
            let unit: CGFloat = pctNumber.count >= 3 ? 1.0 : 1.5
            let glyphW = 3 * unit
            let glyphH = 5 * unit
            let glyphGap: CGFloat = unit
