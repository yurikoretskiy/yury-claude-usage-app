...
enum MenuBarRenderer {
    ...
    static func renderCompactMenuBarImage(percentage: Double) -> NSImage {
        ...
        return NSImage(size: NSSize(width: width, height: height), flippable: false) { _ in
            ...
            // v2.13: transparent body, outline in macOS system label color
            // (adapts to light/dark menu bar), faint alpha so digit is primary.
            NSColor.labelColor.withAlphaComponent(0.5).setStroke()
            path.lineWidth = 1.0
            path.stroke()

            // --- Digits: centered via paragraph style (horizontal) + draw-in-
            //     rect centered on body mid-y (vertical). Simple and robust. ---
            NSGraphicsContext.current?.shouldAntialias = true
            let pctNumber = "\(Int(round(percentage)))"
